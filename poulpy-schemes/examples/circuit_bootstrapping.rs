use poulpy_core::{
    GLWENormalize,
    layouts::{
        GGLWEToGGSWKeyLayout, GGSW, GGSWInfos, GGSWLayout, GGSWOwned, GGSWPreparedOwned, GLWE, GLWEAutomorphismKeyLayout,
        GLWEInfos, GLWELayout, GLWEOwned, GLWEPlaintext, GLWEPlaintextOwned, GLWESecret, GLWESecretOwned,
        GLWESecretPreparedOwned, LWE, LWELayout, LWEOwned, LWEPlaintext, LWEPlaintextOwned, LWESecret, LWESecretOwned,
...
use poulpy_schemes::bin_fhe::{
    ...
    circuit_bootstrapping::{
        CircuitBootstrappingKey, CircuitBootstrappingKeyLayout, CircuitBootstrappingKeyOwned, CircuitBootstrappingKeyPrepared,
        CircuitBootstrappingKeyPreparedOwned,
    },
};
...
fn main() {
    ...
    let mut sk_glwe_prepared: GLWESecretPreparedOwned<BackendImpl> = GLWESecretPrepared::alloc(&module, rank.into());
    ...
    // LWE plaintext
    let mut pt_lwe: LWEPlaintextOwned = LWEPlaintext::alloc(base2k.into(), k_lwe_pt.into());
    ...
    let mut cbt_key: CircuitBootstrappingKeyOwned<CGGI> = CircuitBootstrappingKey::alloc_from_infos(&cbt_infos);
    ...
    let mut cbt_prepared: CircuitBootstrappingKeyPreparedOwned<CGGI, BackendImpl> =
    ...
    let mut ct_glwe: GLWEOwned = GLWE::alloc_from_infos(&glwe_infos);

    // Some GLWE plaintext with signed data
    let k_glwe_pt: usize = 3;
    let mut pt_glwe: GLWEPlaintextOwned = GLWEPlaintext::alloc_from_infos(&glwe_infos);
    ...
    let mut res_prepared: GGSWPreparedOwned<BackendImpl> = GGSWPrepared::alloc_from_infos(&module, &res);
    ...
    // Decrypt
    let mut pt_res: GLWEPlaintextOwned = GLWEPlaintext::alloc_from_infos(&glwe_infos);
